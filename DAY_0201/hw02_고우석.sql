...
#1) 1960년에 노벨상 물리학상 또는 노벨 평화상을 수상한 사람의 정보를 출력
      ...
      from nobel where category= 'physics' or category='Peace';
...
--  출력 컬럼: year,	category,	birth_continent,	birth_country
   ...
   from nobel where fullname='Albert Einstein';
...
-- 수상자의 모든 정보를 수상자 이름을 기준으로 오름차순으로 정렬 후 출력
 ...
 from nobel
 where category != 'Physiology or Medicine' order by fullname  asc;
...
#7)각 분야별 역대 수상자의 수를 내림차순으로 정렬 후 출력
select category,count(*)as 횟수
from nobel 
group by category order by 횟수 desc;





#8) 노벨 의학상이 있었던 연도 모두 출력(distinct) 사용
...
select year,category,prize_amount,fullname,gender,birth_continent,birth_country
from nobel where birth_country ='KOREA';


#13) 수상자의 출신 국가가 (EUROPE,'North America','공백'이 아닌 정보 출력)
...
#14)수상자의 출신 국가별로 그룹화를 해서 
-- 수상 횟수가 10회 이상인 국가의 모든 정보 출력하고 국가별 수상횟수의 역순으로 출력
...
-- 오름차순으로 출력
select distinct fullname,count(*)
from nobel
group by fullname 
...
select *
from nobel
where nobel.birth_country like '%Usa%' and category='Literature'
